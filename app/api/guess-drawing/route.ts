...
import { VISION_MODELS, calculateCost, getModelById } from "@/lib/models";
...
export async function POST(request: Request) {
  try {
    ...
    const modelsToUse = selectedModelIds?.length
      ...
      : VISION_MODELS.slice(0, 4).map((id) => getModelById(id)).filter(Boolean);

    if (modelsToUse.length === 0) {
      return Response.json({ error: "No valid vision models selected" }, { status: 400 });
    }
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        controller.enqueue(
          encoder.encode(
            `data: ${JSON.stringify({
              type: "start",
              totalModels,
              models: modelsToUse.map((m: { id: string; name: string; color: string }) => ({
                id: m.id,
                name: m.name,
                color: m.color,
              })),
        ...
        const guessPromises = modelsToUse.map(async (model: { id: string; name: string; color: string }) => {
          ...
          try {
            ...
            completedCount++;
            console.log(`[${model.id}] Guess: "${fullGuess.trim()}" (${generationTimeMs}ms)`);
        ...
        const totalCost = successfulGuesses.reduce((sum, g) => sum + (g.cost || 0), 0);
