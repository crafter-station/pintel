...
export async function calculateSemanticScore(
  ...
  const [guessEmbedding, answerEmbedding] = await getEmbeddings([guess, answer]);
  const similarity = cosineSimilarity(guessEmbedding, answerEmbedding);
  return Math.max(0, Math.min(1, similarity));
}

export function calculateTimeBonus(timeMs: number, maxTimeMs: number = 60000): number {
...
): { baseScore: number; timeBonus: number; multiplier: number; finalScore: number } {
